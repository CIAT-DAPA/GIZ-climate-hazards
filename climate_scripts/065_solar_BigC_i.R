...
calc_indices_srad <- function(country = 'Burkina_Faso',
                              county  = 'Cascades',
                              iso3c   = 'BFA',
                              adm_lvl = 1,
                              seasons = 1,
  ...
  # Aqui debo de revisar que debo de leer --- al ser radiaci?n deberia ser los archivos normales. 
  # Deberia poner que si no tiene la radiaci?n mande un aviso o pare. 
  if(time == 'past'){
  ...
  climate_index <- fst::fst(paste0(outDir,'/',county,'_',period,'_prec_temp.fst') ) %>% 
    as_tibble() %>% 
    filter(id %in% id_sample)
  #######
  
  all_index <- rad_index %>% 
    dplyr::select(-data) %>% dplyr::select(-semester) %>% 
    dplyr::full_join(climate_index , . ) %>% # 
...
calc_indices_srad(country = 'Burkina_Faso',
                  county  = 'Cascades',
                  iso3c   = 'BFA',
                  adm_lvl = 1,
                  seasons = 1,
