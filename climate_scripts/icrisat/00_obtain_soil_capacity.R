# Soil capacity calculation: ICRISAT climate-hazards
# By: H. Achicanoy
...
shp <- raster::shapefile('//dapadfs.cgiarad.org/workspace_cluster_8/climateriskprofiles/data/shps/Niger/NER_adm0_.shp')
shp1 <- raster::shapefile('//dapadfs.cgiarad.org/workspace_cluster_8/climateriskprofiles/data/shps/Senegal/SEN_adm0_.shp')

coords <- "//dapadfs.cgiarad.org/data_cluster_4/observed/gridded_products/chirps/daily/chirps-v2.0.1981.01.01.tif" %>%
  raster::raster(.) %>%
  raster::crop(., raster::extent(shp)) %>%
  raster::mask(., mask = shp) %>%
  raster::as.data.frame(., xy = T, na.rm = T)
coords1 <- "//dapadfs.cgiarad.org/data_cluster_4/observed/gridded_products/chirps/daily/chirps-v2.0.1981.01.01.tif" %>%
  raster::raster(.) %>%
...
fst::write_fst(soil_data2, '//dapadfs.cgiarad.org/workspace_cluster_8/climateriskprofiles/data/soil_data_icrisat.fst')
...
fst::write_fst(soil_data4, '//dapadfs.cgiarad.org/workspace_cluster_8/climateriskprofiles/data/soilcp_data_icrisat.fst')
