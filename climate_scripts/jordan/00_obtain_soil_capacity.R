# Soil capacity calculation: Jordan climate-hazards
# By: H. Achicanoy
...
r <- '//dapadfs.cgiarad.org/workspace_cluster_8/climateriskprofiles/pptF_RCP85_1980_1.grd' %>%
  raster::raster(.) %>%
  raster::crop(., raster::extent(shp)) %>%
  raster::mask(., mask = shp)

coords <- '//dapadfs.cgiarad.org/workspace_cluster_8/climateriskprofiles/pptF_RCP85_1980_1.grd' %>%
  raster::raster(.) %>%
  raster::crop(., raster::extent(shp)) %>%
  raster::mask(., mask = shp) %>%
  raster::as.data.frame(., xy = T, na.rm = T)

coords$id <- raster::cellFromXY(object = r, xy = coords %>% dplyr::select(x,y) %>% as.data.frame)
...
fst::write_fst(soil_data2, '//dapadfs.cgiarad.org/workspace_cluster_8/climateriskprofiles/data/soil_data_jordan.fst')
...
fst::write_fst(soil_data4, '//dapadfs.cgiarad.org/workspace_cluster_8/climateriskprofiles/data/soilcp_data_jordan.fst')
