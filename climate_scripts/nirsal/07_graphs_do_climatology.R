...
do_climatology <- function(value_chain = 'Maize',
                           ...
                           auto    = list(n = 2)) # Seasons defined by algorithm/automatically
{
  ...
  site <- county %>%
    purrr::map(.f = function(cnt){
      input1 <- paste0(root, "/data/observational_data/",tolower(country),"/",tolower(cnt),".fst")
  ...
  outDir <- paste0(root,'/NIRSAL/Nigeria_',value_chain,'/graphs')
  ...
  if(!seasons){
    ...
    ggplot2::ggsave(filename = paste0(outDir,'/',tolower(value_chain),'_climatology.png'), plot = plt, device = "png", width = 12, height = 6, units = "in")
    ...
    if(!is.null(auto)){
      ...
      ggplot2::ggsave(filename = paste0(outDir,'/',tolower(value_chain),'_climatology_auto_seasons.png'), plot = plt, device = "png", width = 12, height = 6, units = "in")
    } else {
      if(!is.null(manual)){
      ...
      ggplot2::ggsave(filename = paste0(outDir,'/',tolower(value_chain),'_climatology_manual_seasons.png'), plot = plt, device = "png", width = 12, height = 6, units = "in")
    }
